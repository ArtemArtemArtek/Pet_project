...
export const ArticleItemList: React.FC<ArticleItemListProps> = (props) => {
    ...
    const ChangeView = () => {
        setViewArticle(
            viewArticle === ArticlesView.BIG
                ? ArticlesView.SMALL
                : ArticlesView.BIG,
        );
        dispatch(
            articlesActions.setView(
                viewArticle === ArticlesView.BIG
                    ? ArticlesView.SMALL
                    : ArticlesView.BIG,
            ),
    ...
    if (articles.length === 0 && !articlesData.isLoading) {
        return <div className={cls.not_found}>{t('Статьи не найдены')}</div>;
    }

    return (
        <div>
            <Button
                data-testid="ArticleItemChangeViewButton"
                className={cls.button_change}
                theme={ButtonTheme.CLEAR}
                onClick={ChangeView}>
                {viewArticle === ArticlesView.BIG ? (
                    <Tiles className={cls.view_icons} />
                ) : (
