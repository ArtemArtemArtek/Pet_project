...
export const SortArticleOptions: React.FC<SortArticleOptionsProps> = (
    props,
) => {
    const { sortFieldsOptions, sortOrderOptions, defaultSearchParams, cards } =
        props;
    ...
    const changeSortField = (e: React.ChangeEvent<HTMLSelectElement>) => {
        dispatch(
            articlesActions.setSortField(e.target.value as ArticleSortFields),
        );
    ...
    return (
        <section className={cls.filtersWrapper}>
            <div>
                    ...
                    {sortFieldsOptions.map((element, id) => {
                        return (
                            <option
                                ...
                                key={id}
                                selected={
                                    sort_field == element.value ? true : false
                                }>
                                {element.title}
                            </option>
                        );
                    ...
                    {sortOrderOptions.map((element, id) => {
                        return (
                            <option
                                ...
                                key={id}
                                selected={
                                    order == element.value ? true : false
                                }>
                                {element.title}
                            </option>
                        );
                ...
                {t('Найти')}
                <Input
                    data-testid="ArrticlesSearchInput"
                    className={cls.select_field}
                ...
                {cards.map((el) => {
                    return (
                        <Button
                            data-testid={el}
                            className={
                                selectTab === el ? cls.selectedTab : cls.tab
                            }
