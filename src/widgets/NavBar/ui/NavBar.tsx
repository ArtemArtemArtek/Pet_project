...
const ModalLazy = React.lazy(() =>
    import('../../../features/AuthModal').then((module) => ({
        default: module.AuthModal,
    })),
);
const LogoutModalLazy = React.lazy(() =>
    import('../../../features/LogoutModal').then((module) => ({
...
const NavBar: React.FC<NavBarProps> = ({ className }) => {
    ...
    const isAdmin =
        isAuth?.role.includes(UserRoles.ADMIN) ||
        isAuth?.role.includes(UserRoles.MANAGER);
    ...
    return (
        <div className={ClassNameHelper(cls.navbar, {}, [className])}>
            <h1 className={cls.main_Logo_Sign}>{t('Artem Rumbens')}</h1>
            <div className={cls.links}>
                {isAuth ? (
                    <div className={cls.wrapper_actions}>
                        <Notifications
                            className={cls.notification_position}
                        ...
                        />
                        <LogoutModalLazy
                            className={ClassNameHelper(cls.logoutModal)}
                            opened={openedLogoutModal}
                            setOpen={() => setIsOpenedLogoutModal(false)}
                            logOut={() => logOut()}
                        />
                    </div>
                ) : (
                    <div>
                        ...
                        {opened && (
                            <ModalLazy
                                className={ClassNameHelper(cls.authModal, {}, [
                                    className,
                                ])}
                                opened={opened}
                                setOpen={() => setIsOpened(false)}
                            />
                        )}
                    </div>
                )}
